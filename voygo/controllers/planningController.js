...
function syncTripInputs() {
  ...
  if (peopleInput) peopleInput.value = String(tripState.people || 1);
  if (destinationInput) {
  ...
  if (descriptionInput) descriptionInput.value = tripState.description || '';
...
function initTripEditor() {
  ...
  if (!nameInput || !peopleInput || !destinationInput || !startInput || !endInput || !descriptionInput || !saveButton || !saveNote) return;

  if (isReadOnlyTrip()) {
    saveButton.hidden = true;
    [nameInput, peopleInput, destinationInput, startInput, endInput, descriptionInput].forEach((input) => {
  ...
  saveButton.addEventListener('click', async () => {
    ...
    const nextPeople = parseInt(peopleInput.value, 10);
    ...
    if (!Number.isFinite(nextPeople) || nextPeople < 1) {
      saveNote.classList.add('is-error');
      saveNote.textContent = 'Le nombre de personnes doit etre au moins de 1.';
      return;
    }
